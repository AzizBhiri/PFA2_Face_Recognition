...
def index():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            NEWPATH = videotest(os.path.join(
...
def videotest(filename):
    ...
    aziz = face_recognition.load_image_file("aziz.jpg")
    encoding = face_recognition.face_encodings(aziz)[0]
    ...
    for i in range(1, length-1):
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            cv2.rectangle(frame, (left, bottom - 10),
                          (right, bottom + 10), (10, 10, 10), cv2.FILLED)
